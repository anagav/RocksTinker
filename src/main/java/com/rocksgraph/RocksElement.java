...
public abstract class RocksElement implements Element {
    ...
    public RocksElement(final Object id, final String label, RocksGraph graph, Object[] keyValues) {
        this.graph = graph;
    ...
    protected Iterator innerPropertyIterator(String[] propertyKeys) {
        ...
        if (propertyKeys.length > 0)
            return properties.entrySet().stream()
                    .filter(entry -> ElementHelper.keyExists(entry.getKey(), propertyKeys)).map(Map.Entry::getValue).iterator();
